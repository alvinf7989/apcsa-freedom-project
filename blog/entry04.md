...
```
To explain, this pellet code initializes the speed of it which is okay with me, and also creates the function that allows the pellet to move up with `position.y`. But before I did that, I had to add the `shoot()` function in my player script, so the pellet knows where to be shot from.
```java
func shoot():
...
```
This is to ensure that the pellets aren't at the top right, I set up the global position of the pellets to be aligned with the bottom ship. To actually have this function work I used a condition called `if Input.is_action_just_pressed("ui_accept"):`, where `ui_accept` is the same as pressing `enter` or `spacebar`. Now to have these scenes connected I put on the top
```java
